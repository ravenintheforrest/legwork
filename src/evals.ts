...
// in memory — it never touches data/ — and scores them against the authored fixture regression
...
export async function runEvals(opts: EvalOptions = {}): Promise<void> {
  ...
  const humanCount = golden.filter((row) => row.labeled_by && row.labeled_by !== "bootstrap").length;
  console.log(`evaluation scope: fixture regression (${golden.length} rows; ${humanCount} human-adjudicated), not live accuracy`);
...
// discover -> discover-jobs -> discover-issues -> discover-gitlab -> resolve -> enrich -> dedupe ->
// qualify -> people.
async function scoreFixtures(
  ...
  state = mergeAccounts(state, await AGENTS["discover-issues"]!.run(state, ctx));
...
function readGolden(file: string): GoldenRow[] {
  ...
  return parseJsonl<GoldenRow>(readFileSync(file, "utf8"), file);
}
