...
export interface IssueItem {
  ...
  state?: string;
  created_at?: string;
  /** The author — discover-issues reads the login and nothing else from here. */
...
export class GitHubClient {
  ...
  private readonly injectedHttp: HttpClient | undefined;
  ...
  async searchIssues(org: string, query: string, opts: { perPage?: number } = {}): Promise<IssueSearch> {
    if (this.mode === "fixture") {
      const file = join(this.fixtureDir, "github", "issues", `${org}.json`);
      return readJson<IssueSearch>(file) ?? { total_count: 0, items: [] };
    }
    const perPage = opts.perPage ? `&per_page=${Math.min(100, Math.max(1, Math.floor(opts.perPage)))}` : "";
    const body = await this.get<IssueSearch>(`${API}/search/issues?q=${encodeURIComponent(query)}${perPage}`);
    return body ?? { total_count: 0, items: [] };
  }
  ...
  private async get<T>(url: string): Promise<T | null> {
    ...
    const maxAgeMs = cacheMaxAge(url, cached?.status);
    if (cached && Date.now() - Date.parse(cached.fetched_at) < maxAgeMs) {
      return cached.status === 404 ? null : (cached.body as T);
    }

    const headers: Record<string, string> = { Accept: "application/vnd.github+json" };
    // Credentials never leave GitHub, whatever a caller passes in.
    if (this.token && isGitHubHost(url)) headers.Authorization = `Bearer ${this.token}`;
    // 404/204 come back for caching; every other non-2xx throws out of the policy layer
    // with host, status, attempt count, and total wait already in the message.
    const response = await this.http().request(url, { headers, accept: [404, 204] });
...
function cacheMaxAge(url: string, status?: number): number {
  if (status === 404 || status === 204) return 60 * 60_000;
  if (url.includes("/search/")) return 60 * 60_000;
  if (url.includes("/issues")) return 2 * 60 * 60_000;
  return 24 * 60 * 60_000;
}
