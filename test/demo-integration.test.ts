...
function ensureReplayBrief(dir: string, first: CliResult): string | null {
  ...
  for (const account of accountsIn(dir)) {
    ...
    const body = [
      ...
      "",
      "## Who",
      `${account.company ?? account.org} ships a mobile app.`,
      "",
      "## Production Expo signals",
      ...allowed.slice(0, 3).map((url, i) => `- signal ${i + 1} ([source](${url}))`),
      "",
      "## Who to talk to",
      "- Someone on the mobile team.",
      "",
      "## Suggested opener",
      "Public evidence that they build with Expo.",
      "",
    ].join("\n");
...
function fixtureFor(dir: string, urls: string[]): string {
  const fixtureDir = join(dir, "fixtures", "llm");
  const match = readdirSync(fixtureDir)
  ...
  assert.ok(match, "no replay fixture matched the brief's receipts");
  return join(fixtureDir, match);
}
...
test("the citations gate end to end: a corrupted replay fixture is caught, and restoring is clean", async (t) => {
  await withWorkingCopy((dir) => {
    ...
    const fixturePath = fixtureFor(dir, urls);
    const originalFixture = readFileSync(fixturePath, "utf8");
    const saved = JSON.parse(originalFixture) as { text: string };
    const corrupted = saved.text.replace(/\]\((https?:[^)]+)\)/, `](${FABRICATED})`);
    assert.notEqual(corrupted, saved.text, "the fixture must actually change");
    writeFileSync(fixturePath, JSON.stringify({ ...saved, text: corrupted }, null, 2) + "\n");
