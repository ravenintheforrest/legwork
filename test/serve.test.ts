...
const PANELS = ["overview", "queue", "briefs", "evals", "memos", "runs"];
...
test("the desk serves its console, every panel, and every action", async (t) => {
  await withDesk(async (desk, dir) => {
    ...
    // --- POST /api/notify ----------------------------------------------------
    const notifyNoOrg = await call(desk.base, "POST", "/api/notify", {});
    assert.equal(notifyNoOrg.status, 400);

    const notifyMissing = await call(desk.base, "POST", "/api/notify", { org: "not-an-org" });
    assert.equal(notifyMissing.status, 404);

    const published = accountsIn(dir).find((a) =>
      existsSync(join(dir, "briefs", `${a.org}.slack.txt`)),
    );
    assert.ok(published, "the demo must publish at least one brief for notify to reach");
    const notified = await call(desk.base, "POST", "/api/notify", { org: published.org });
    assert.equal(notified.status, 200, notified.text);
    // Offline, with no webhook configured: it prints rather than posting.
    assert.equal((notified.body as { result: string }).result, "printed");
