...
function seed(dir: string): Account[] {
  ...
  for (const org of ["alpha", "bravo"]) {
    ...
    writeFileSync(join(dir, "briefs", "queue", `${org}.summary.txt`), `*${org}* queued\n`);
...
test("approving appends a decision, flips the account, and publishes the brief", async () => {
  await withWorkingCopy((dir) => {
    ...
    assert.equal(existsSync(join(dir, "briefs", "queue", "alpha.summary.txt")), false);
    assert.ok(existsSync(join(dir, "briefs", "alpha.md")));
    assert.ok(existsSync(join(dir, "briefs", "alpha.summary.txt")));
...
test("approval has no implicit outbound notification side effect", async () => {
  await withWorkingCopy((dir) => {
    seed(dir);
    const result = runCli(dir, ["review", "--approve", "alpha"]);
    assert.equal(result.status, 0, result.stderr);
    assert.ok(!/Slack|posted|webhook/i.test(result.stdout));
    assert.ok(existsSync(join(dir, "briefs", "alpha.decision.json")));
  });
});
