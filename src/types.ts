...
export interface Account {
  ...
  /** The org profile's location, as GitHub shows it — orgs only. A person's location is
   *  never copied onto an account (see people.ts): a company HQ is public, a home is not. */
  location?: string;
  /** Which world this record came from. A live run and a fixture run must never mix:
   *  fixture accounts carry authored receipts that do not resolve. */
  mode?: "live" | "fixture";
  ...
  pipeline?: PipelineState;     // freshness/invalidation metadata owned by the runner
}
...
export interface RunRecord {
  ...
  /** Live HTTP budget for this unit: requests, retries, rate-limit hits, wait time.
   *  Absent when the unit made no live request (every fixture run, and cached live runs). */
  http?: HttpUsage;
...
export interface RunContext {
  ...
  /** Where fixture-mode clients built by agents (web, HN, job boards) read from. Default "fixtures". */
  fixtureDir?: string;
}
