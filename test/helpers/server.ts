...
const deskTokens = new Map<string, string>();
  ...
  for (;;) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!token) throw new Error("desk page did not bootstrap a request token");
        deskTokens.set(base, token);
        return { base, token, child, output: () => output, stop };
      }
    } catch {
      // not listening yet
    }
...
export async function call(
  ...
  const token = deskTokens.get(base) ?? "";
  const headers: Record<string, string> = { "x-legwork-token": token };
