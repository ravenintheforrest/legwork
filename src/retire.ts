...
const RETIRABLE = new Set(["discover", "discover-jobs", "discover-gitlab"]);
...
export function runRetire(agent: string): void {
  ...
  if (!RETIRABLE.has(agent)) {
    throw new Error(`retirement attribution is only defined for discovery agents: ${[...RETIRABLE].join(", ")}`);
  }
...
function attribute(agent: string, accounts: Account[]): Attribution {
  ...
  const discoveryAgents = RETIRABLE;
