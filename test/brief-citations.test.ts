...
import { inDir, removeTemp, workingCopy } from "./helpers/env.js";
...
function body(urls: string[], opts: { drop?: string } = {}): string {
  const cite = (claim: string, index: number) => `${claim} ([source](${urls[index % urls.length]}))`;
  const sections: Record<string, string[]> = {
    "## Who": [cite("Acme builds a consumer mobile app.", 0)],
    "## Production Expo signals": urls.map((url, i) => `- signal ${i + 1} ([source](${url}))`),
    "## Who to talk to": [`- ${cite("Someone on the mobile team.", 1)}`],
    "## Suggested opener": [cite("Public evidence that Acme builds with Expo.", 2)],
  };
  return [
    "# Acme — account brief",
    "",
...
test("THE GATE: a factual model line without its own receipt is rejected", async () => {
  const uncited = body([EAS_MAIN, STORE, CAREERS]).replace(
    `Acme builds a consumer mobile app. ([source](${EAS_MAIN}))`,
    "Acme builds a consumer mobile app.",
  );
  const { decision } = await runGate(new StubLLM(uncited));
  assert.equal(decision.brief_mode, "template");
  assert.match(decision.llm?.reject_reason ?? "", /^claim lacks an evidence receipt:/);
});
...
test("fewer than three receipts is rejected", async () => {
  ...
  assert.equal(decision.brief_mode, "template");
  assert.equal(decision.llm?.reject_reason, "fewer than 3 distinct receipts");
});
...
test("the pinned test model stays priced", () => {
