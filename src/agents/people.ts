  ...
  async run(input: Account[], ctx: RunContext): Promise<Account[]> {
    ...
    for (const account of input) {
      ...
      for (const contributor of humans) {
        ...
        const facts = profileFacts(profile, process.env.LEGWORK_INCLUDE_PERSONAL_PROFILE_FIELDS === "true");
        if (facts.length > 0) {
          fresh.push({
...
function profileFacts(profile: OrgProfile | null, includePersonal: boolean): string[] {
  if (!profile) return [];
  const facts: string[] = [];
  ...
  // Location and free-text bio are public but not necessary for the default GTM task.
  // Retaining them requires an explicit responsible-use opt-in.
  if (includePersonal && location) facts.push(`location '${location}'`);
  if (includePersonal && bio) facts.push(`bio '${bio.length > MAX_BIO_CHARS ? bio.slice(0, MAX_BIO_CHARS - 1).trimEnd() + "…" : bio}'`);
  return facts;
}
