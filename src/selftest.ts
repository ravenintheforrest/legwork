...
function modelBody(urls: string[], drop?: string): string {
  ...
  const sections: [string, string[]][] = [
    ["## Who", [cite("Acme ships a consumer mobile app.", 0)]],
...
const selftestDeskTokens = new Map<string, string>();
  ...
  for (;;) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const token = /window\.__LEGWORK_TOKEN__="([^"]+)"/.exec(html)?.[1];
        if (!token) throw new CheckFailed("desk did not bootstrap a request token");
        selftestDeskTokens.set(base, token);
...
async function ask(
  ...
  const headers: Record<string, string> = { "x-legwork-token": selftestDeskTokens.get(base) ?? "" };
...
const CHECKS: Check[] = [
  ...
  },
  {
    ...
    async run() {
      ...
      eq(thin.decision.llm?.reject_reason, "fewer than 3 distinct receipts", "thin receipts not caught");
    },
  },
  {
  ...
  },
  {
    ...
    async run() {
      ...
      for (const org of ["alpha", "bravo"]) {
        ...
        writeFileSync(join(dir, "briefs", "queue", `${org}.summary.txt`), `*${org}*\n`);
      }
  ...
  },
  {
    ...
    async run() {
      ...
      try {
        ...
        for (const panel of ["inbox", "accounts", "search", "system"]) {
