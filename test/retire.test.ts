...
function registryDoc(threshold: number): unknown {
  return {
    ...
    agents: {
      ...
      "discover-jobs": {
        does: "find public job-post signals",
        output: "candidates.jsonl",
        hypothesis: "job posts add accounts code search misses",
      },
      qualify: { does: "score candidates", output: "qualified.jsonl" },
    },
...
test("discover-jobs receives unique-discovery attribution", async () => {
  const accounts = Array.from({ length: 5 }, (_, i) => account({
    org: `job-${i}`,
    stage: "briefed",
    evidence: [evidence("job signal", `https://example.com/job-${i}`, "discover-jobs")],
  }));
  const { memo } = await retireIn({
    threshold: 0.5,
    accounts,
    runs: [run("discover-jobs")],
    agent: "discover-jobs",
  });
  assert.ok(memo.includes("verdict: **KEEP**"));
  assert.ok(memo.includes("**5 briefs would not exist without it**"));
});

test("retirement refuses a non-discovery agent", async () => {
  await withTempDir(async (dir) => {
    writeFileSync(join(dir, "registry.yaml"), yaml.dump(registryDoc(0.05)));
    mkdirSync(join(dir, "data"), { recursive: true });
    writeFileSync(join(dir, "data", "runs.jsonl"), JSON.stringify(run("qualify")) + "\n");
    writeFileSync(join(dir, "data", "accounts.jsonl"), "");
    await inDir(dir, () => {
      assert.throws(() => runRetire("qualify"), /only defined for discovery agents/);
    });
    assert.equal(existsSync(join(dir, "memos")), false);
  });
});
